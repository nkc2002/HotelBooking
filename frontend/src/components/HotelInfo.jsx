...
const amenityIcons = {
  wifi: { icon: Wifi, label: "Wi-Fi miễn phí" },
  parking: { icon: Car, label: "Bãi đỗ xe miễn phí" },
  pool: { icon: Waves, label: "Hồ bơi" },
  gym: { icon: Dumbbell, label: "Phòng gym" },
  restaurant: { icon: Utensils, label: "Nhà hàng" },
  spa: { icon: Wind, label: "Spa và chăm sóc sức khỏe" },
  breakfast: { icon: Coffee, label: "Bao gồm bữa sáng" },
  pet: { icon: Dog, label: "Thân thiện với thú cưng" },
};
...
const HotelInfo = ({ hotel }) => {
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      text: `Xem khách sạn tuyệt vời này: ${hotel?.name || "Grand Luxury Resort & Spa"}`,
  ...
  const displayLocation = [address, city].filter(Boolean).join(", ");
      ...
      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-4 relative">
          ...
          >
            ...
            <span className="underline">{isSaved ? "Đã lưu" : "Lưu"}</span>
          </button>
          {showShareToast && (
